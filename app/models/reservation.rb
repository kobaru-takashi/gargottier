class Reservation < ApplicationRecord
  ...
  validates :family_name, :first_name, presence: { message: 'は１文字以上入力してください。' }, format:{ with: /\A(?:\p{Hiragana}|\p{Katakana}|[ー－]|[一-龠々])+\z/ }
  validates :family_name_kana,:first_name_kana, presence: { message: 'は１文字以上入力してください。' }, format:{with: /\A([ァ-ン]|ー)+$\z/}
  validates :phone_number, presence: true, format:{with: /\A\d{10,11}\z/}
  VALID_EMAIL_REGEX = /\A[\w+\-.]+@[a-z\d\-.]+\.[a-z]+\z/i
  validates :email, presence: true, format: { with: VALID_EMAIL_REGEX }
  validates :request, length: { maximum: 200 }
end
